...
export default function ProfitLossChart({ data }: ProfitLossChartProps) {
  ...
  // Daily Revenue vs Expenses Chart (Combined Bar + Line like the image)
  const dailyRevenueExpensesOptions = {
    responsive: true,
    maintainAspectRatio: false,
    plugins: {
      legend: {
      ...
      title: {
        display: true,
        text: `Daily Revenue vs Expenses - ${getPeriodLabel(period)}`,
        font: {
          size: 16,
          weight: 'bold' as const,
        },
  ...
  const dailyRevenueExpensesData = {
    labels: chartData.map((item) => formatDate(item.date)),
    datasets: [
      {
        type: 'bar' as const,
