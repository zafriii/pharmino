...
export default function SingleItemInventoryForm({
  ...
  const onSubmit = async (data: InventoryFormData) => {
    if (
      !receivedItem.canAddToInventory ||
      receivedItem.remainingQuantity <= 0 ||
      receivedItem.isFullyProcessed
    ) {
      setToastMessage({
        message: 'This item has already been fully added to inventory.',
        type: 'error',
      ...
      >
        ...
        >
          ...
          <div className="mb-4 p-4 bg-gray-50 rounded-lg">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 rounded-[8px] bg-[#C0C0C0] overflow-hidden flex items-center justify-center">
                {receivedItem.purchaseItem.item.imageUrl ? (
                  ...
                  <span className="text-xs font-medium text-gray-600">
                    {receivedItem.purchaseItem.item.itemName
                      .charAt(0)
                      .toUpperCase()}
                  </span>
                )}
              ...
              <div>
                ...
                {receivedItem.purchaseItem.item.brand && (
                  <p className="text-sm text-gray-600">
                    Brand: {receivedItem.purchaseItem.item.brand}
                  </p>
                )}
              </div>
            </div>
            <div className="flex gap-2">
              <Badge variant="white">
                {receivedItem.purchaseItem.item.category.name}
              </Badge>
              <Badge variant="blue">
                PO: {receivedItem.purchaseItem.purchaseOrder.id}
              </Badge>
            </div>
          </div>
