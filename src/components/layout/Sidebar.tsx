...
// Memoized menu items - won't recreate on every render
const getMenuItems = (): MenuItem[] => [
  { name: "Dashboard", iconName: "dashboard", path: "/dashboard-overview" },
  { name: "Analytics", iconName: "analytics", path: "/analytics/profit-loss" },
  { name: "Point Of Sale (POS)", iconName: "pos", path: "/sale/pos" },
  { name: "All Sales", iconName: "sales", path: "/sale/all-sale" },
  { name: "Payments", iconName: "payments", path: "/payments" },
  { name: "User & roles", iconName: "users", path: "/admin/hr/directory" },
  { name: "Products", iconName: "products", path: "/admin/product-management/products" },
  {
    name: "Purchases",
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen }) => {
  ...
  const menuItems = useMemo(() => {
    if (userRole === "ADMIN") {
      ...
      // Non-admin users can only see specific items
      const allowedPaths = [
        "/dashboard-overview",
        "/sale/pos",           // POS
        "/sale/all-sale",      // All Sales
        "/payments",           // Payments
        "/inventory",          // Inventory
  ...
  const renderMenuItem = useCallback((item: MenuItem, index: number, isSubItem = false) => {
    ...
    return (
      ...
      >
        <span
          className={`${isActive ? "text-white" : "text-[#27272A] group-hover:text-[#4a90e2]"
            }`}
        >
          <Icon name={item.iconName} size={isSubItem ? 18 : 20} />
        </span>
